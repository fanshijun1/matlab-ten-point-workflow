function [ CU,channel,user,mean_makespan,temp_clock,temp_usernum,temp_makespan] = mainregion(taskCU,CUtask, CULink,auxpreSucc,PCP,I,Scu,K,auxD,B,arrate,USENUM )
%MAIN 此处显示有关此函数的摘要
%   此处显示详细说明
%%%%%%%%%%%%%%%%%%%%变量区%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
clock=1;
step=1;%步进幅度
% USENUM=userAssign; %表示总的用户数，不是系统容纳的用户数500.
                               ...
                               waitQueue=channel(i).channel(j).wait_task_queue;
                                for m=1:length(waitQueue)
                                      ...
                                      if(auxD(curTask(1,2), curTask(1,3))==1)                                              
                                               ...
                                               user(curTask(1,1)).makespan=user(curTask(1,1)).finish_time-user(curTask(1,1)).start_time+user(curTask(1,1)).wait_time;
...
% plot(temp_usernum,temp_makespan,'g');
% hold on ;
% for i=1:length(I)
...
mean_makespan=0;
for i=1:USENUM
        mean_makespan=mean_makespan+user(i).makespan;
end
mean_makespan=mean_makespan/USENUM;
end
% 
% end
