function [ CU,channel,user,mean_makespan,a] = mainten(taskCU,CUtask, CULink,auxpreSucc,PCP,I,Scu,K,auxD,B )
%MAIN 此处显示有关此函数的摘要
%   此处显示详细说明
%%%%%%%%%%%%%%%%%%%%变量区%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
clock=1;
...
capacity=500;
a=[];
mean_makespan=zeros(1,5);
arrate_array=0.004:0.004:0.02
for aa=1:5
...
for i=1:1000%500个用户
    ...
    user(i).finished_task_flag=zeros(1,length(I));%任务传输完成，则标记为1 
    for j=1:length(I)
                ...
                if(isempty(CU(i).slice(j).wait_task_queue)==0)
                   %第1部分虚节点入信道
                    ...
                    while(circle<length(CU(i).slice(j).wait_task_queue)&&I(CU(i).slice(j).wait_task_queue{1,CU(i).slice(j).next_point}(1,2))==0)%如果是虚拟节点，计算量为零，直接部署到通信信道，并挑下一个任务
                            ...
                            if(isempty(user(i).task(CU(i).slice(j).wait_task_queue{1,CU(i).slice(j).next_point}(1,2)).succ)==0)
                                    for k=1:length(user(i).task(CU(i).slice(j).wait_task_queue{1,CU(i).slice(j).next_point}(1,2)).succ)
                                            channel(i).channel(user(CU(i).slice(j).wait_task_queue{1,CU(i).slice(j).next_point}(1,1)).task(user(i).task(CU(i).slice(j).wait_task_queue{1,CU(i).slice(j).next_point}(1,2)).succ(1,k)).CUAssign).wait_task_queue{1,end+1}=[CU(i).slice(j).wait_task_queue{1,CU(i).slice(j).next_point},user(i).task(CU(i).slice(j).wait_task_queue{1,CU(i).slice(j).next_point}(1,2)).succ(1,k)];
                                            %更新信道历史信息
...
%%%%%%%%%%%%%数据传输递减%%%%%%%%%%%%%%%%%%%%%%    
    for i=1:size(CULink,1)
                        ...
                        if(isempty(channel(i).channel(j).wait_task_queue)==0)
                               waitQueue=channel(i).channel(j).wait_task_queue;
                                for m=1:length(waitQueue)
                                      ...
                                      user(curTask(1,1)).finished_task_flag(1,curTask(1,2))=1;
                                      %如果是终结点
                                      if(auxD(curTask(1,2), curTask(1,3))==1)                                              
                                              ...
                                              %后继节点加入到计算刀片
                                              curTask(:,2)=[];
                                              if(all(user(curTask(1,1)).finished_task_flag(user(curTask(1,1)).task(curTask(1,2)).pre))==1)
                                                    %选择wait_taskI最小的刀片
        ...
        %                     end
                            ...
                            succTask(:,2)=[];
                            if(all(user(succTask(1,1)).finished_task_flag(user(succTask(1,1)).task(succTask(1,2)).pre))==1)
                                    %选择wait_taskI最小的刀片
...
mean_makespan(aa)=mean_makespan(aa)/1000;
