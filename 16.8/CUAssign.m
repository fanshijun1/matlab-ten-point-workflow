function [ taskCU,CUtask,auxtask,auxpreSucc,auxD,TT ] = CUAssign( CULink,preSucc,PCP,I,Scu,K,D,B)
    ...
    for q=1:length(PCP{2,p})%对每一个节点                 
        ...
        if(any(pre)==1)%前驱不空
            for k=1:length(pre)%pre(k)和 PCP{2,p}(1,q)的TRT
                ...
                if(B(n,sm)==0&&D(pre(k),PCP{2,p}(1,q))>1)
                    sub=max(n,sm)-min(n,sm)-1%sub表示虚拟节点的个数
                    for i=1:sub
                        I(end+1)=0;
                       ...
                       if(i>1&&i<sub)
                        ...
                        if(i==sub&&sub~=1)
                            ...
                            auxpreSucc{1,length(I)}=length(I)-1;
                            auxpreSucc{2,length(I)}=PCP{2,p}(1,q);
                            auxpreSucc{1,PCP{2,p}(1,q)}(1,find(auxpreSucc{1,PCP{2,p}(1,q)}==pre(k)))=length(I);
