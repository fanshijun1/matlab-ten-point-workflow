...
AR_array=[0.001 0.002 0.003 0.005 0.01:0.01:0.05];
% AR_array=0.05;
...
for i=1:length(find(ap_data_trantime~=0))
    ...
    du=[];
    for j=1:length(ap_channel_task{x,y})
        du(end+1)=length(K{2,floor(ap_channel_task{x,y}(j)/100)});
    end
    temp2(end+1)=max(du);
    ren_in_channel=zeros(Nap,Nap);
    delay_trans=zeros(Nap,Nap);
...
for i=1:length(find(ap_data_trantime~=0))
    ...
    for j=1:length(ap_channel_task{x,y})
        du(end+1)=length(K{2,floor(ap_channel_task{x,y}(j)/100)});
        du(end+1)=length(K{2,mod(ap_channel_task{x,y}(j),100)});
    end
    beta=mean(du);
    ren_in_channel=zeros(Nap,Nap);
    delay_trans=zeros(Nap,Nap);
...
plot(AR_array1,value1,'v-r')
hold on
plot(AR_array1,value5,'^-r')
hold on
plot(AR_array1,value2,'v-b')
hold on
plot(AR_array1,value6,'^-b')
hold on
plot(AR_array1,value3,'v-g')
hold on
plot(AR_array1,value7,'^-g')
hold on
plot(AR_array1,value4,'v-y')
hold on
plot(AR_array1,value8,'^-y')
hold on
plot(AR_array,makespan,'x-c')
hold on

legend('通信慢,方法1,无绿线','通信慢,方法1,加绿线','通信慢,方法2,无绿线','通信慢,方法2,加绿线','通信慢,方法3,无绿线','通信慢,方法3,加绿线','通信快,无绿线','通信快,加绿线','makespan')
xlabel('到达率')
ylabel('时间（s）')
