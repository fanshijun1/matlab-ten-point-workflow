...
for i=1:size(Transferdataq,2)
    K{1,i}=(find(Transferdataq(:,i)))';%前节点,
    K{2,i}=find(Transferdataq(i,:));%后节点，前后驱节点都变成是横向的
end   %计算直接前驱后驱任务
...
task_in_channel=[];
for i=2:size(Map,1)
    suc=K_g{2,i};%绿线新形成的前后驱关系一定在同一个AP，没有传输延时，这里用K和K_g都一样
...
task_start_time=zeros(50000,size(Transferdataq,1));%放入任务的前驱都执行完的clock
task_start_timesign=zeros(50000,size(Transferdataq,1));
task_wait_time=zeros(50000,size(Transferdataq,1));%放入任务前驱都执行完，并且已经分配了vm的开始时间
    ...
    for z=1:ren
        ...
        for i=1:length(vir_resttask)
            ...
            if isempty(find(pretaskrei))==1&&isempty(find(pretasktrans))==1
                ...
                suc_id=VM2Task(1).Task(vir_resttask(i)).suc;%后驱任务的id
                ...
                if AP(Map_AP).CS(choice).queue(vm1).state==0
                    AP(Map_AP).CS(choice).queue(vm1).state=1;%预备状态，程序执行的这个clock当中，并不执行,当这一时刻传输处理完成之后，变为1。此时rear==front==1，加入数据后，rear依然指向队尾，front依然指向队头
                else
                    AP(Map_AP).CS(choice).queue(vm1).rear=AP(Map_AP).CS(choice).queue(vm1).rear+1;%当还有其他的data到达的时候，rear指向最后一个data
                end
                if ap==7&&cs==1&&vm==5
                end
